...
Permission is hereby granted, at a charge of ${price}STYC (TR3X), payable to ${EVM_CHAIN_NAMES[evmChainId]} chain address ${payee}, to any person purchasing a token of this digital license asset to perform the associated track named "${title}", © ${copyrightYear} ${commaList(artists, "and")}, identified by its BLAKE3 256-bit hash digest 0x${blake3256}, in public, for a lease term of ${term} finalized blocks on the ${EVM_CHAIN_NAMES[evmChainId]} chain, starting with the block number that the purchase transaction acquiring this license got finalized in.
...
The artist name${artists.length > 1 ? "s" : ""} "${commaList(artists, "and")}" must be visibly included in all digital and physical copies and noticeably mentioned at any public performances explicitely accrediting ${commaList(artists, "and")} as the creator${artists.length > 1 ? "s" : ""} of "${title}".

Claims of this license must be prooved using tr3x purchase transactions on the ${EVM_CHAIN_NAMES[evmChainId]} chain.
      `.trim()
    };
...
Permission is hereby granted, at a charge of ${price}STYC (TR3X), payable to ${EVM_CHAIN_NAMES[evmChainId]} chain address ${payee}, to the first person purchasing a token of this digital license asset to exclusively perform the associated track named "${title}", © ${copyrightYear} ${commaList(artists, "and")}, identified by its BLAKE3 256-bit hash digest 0x${blake3256}, in public.

The artist name${artists.length > 1 ? "s" : ""} "${commaList(artists, "and")}" must be visibly included in all digital and physical copies and noticeably mentioned at any public performances explicitely accrediting ${commaList(artists, "and")} as the creator${artists.length > 1 ? "s" : ""} of "${title}".

Claims of this particular license must be verified against their respective purchases on the ${EVM_CHAIN_NAMES[evmChainId]} chain.
    `.trim()
    };
...
tape("blake3256 some data possibly in the browser using wasm", async (t) => {
    await blake3();
    t.equal(blake3hash256hex(Uint8Array.from(Buffer.from("fraud world"))), "02cb5a8d8d1c78b28217b8f8dc0230353c45afb92395af643239e38e1d9c1420", "blake3hash256hex");
});
...
tape("ipfs add & cat", async (t) => {
    const ipfs = createIpfsPinrClient(`http://${process.env.ALB_DOMAIN_NAME}`, `https://${process.env.DIST_DOMAIN_NAME}`);
